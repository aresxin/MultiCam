...
class ViewController: UIViewController, AVCaptureAudioDataOutputSampleBufferDelegate, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    // MAKR: My
    @IBOutlet weak var metalContainer: UIView!
    var frontMtkView: DragMTKView!
    var backMtkView: MTKView!
    var frontRenderer: MetalRenderer!
    var backRenderer: MetalRenderer!

    var photoImageView: UIImageView!

    var frontVideoImage: CIImage?
    var frontVideoMaskImage: CIImage?
    var backVideoImage: CIImage?

    var mix: MixMode = .depth
    var backRendererImage: CIImage?
    var frontRendererImage: CIImage?


    // MetalVideoRecorder
    var metalRecorder: MetalVideoRecorder?
    ...
    let serialQueue = DispatchQueue(label: "com.Camera-Depth-Sampler.queue")
	...
	override func viewDidLoad() {
        ...
        metalRecorder = MetalVideoRecorder(outputURL: URL(string: "www.baidu.com")!, size: CGSize(width: w, height: h))
...
//		frontCameraVideoPreviewLayer = frontCameraVideoPreviewView.videoPreviewLayer
	...
	private func configureBackCamera() -> Bool {
        ...
        do {
            ...
            backCamera.setFocusModeLocked(lensPosition: 1, completionHandler: nil)
            backCamera.unlockForConfiguration()
        } catch {
            backCamera.unlockForConfiguration()
        }

        // My Connect AVCapturePhotoOutput
...
//        backPhotoOutput.isDepthDataDeliveryEnabled = backPhotoOutput.isDepthDataDeliverySupported
//        backPhotoOutput.enabledSemanticSegmentationMatteTypes = backPhotoOutput.availableSemanticSegmentationMatteTypes
//        let backPhotoConnection = AVCaptureConnection(inputPorts: [backCameraVideoPort], output: backPhotoOutput)
...
//		session.addConnection(backCameraVideoPreviewLayerConnection)
	...
	private func configureFrontCamera() -> Bool {
        ...
        frontVideoConnection = frontCameraVideoDataOutput.connection(with: .video)

        return true
        //frontCamera.selectDepthFormat()

        
        // depth output
        guard session.canAddOutput(depthDataOutput) else {
            print("Could not add the depthDataOutput")
            return false
        }
        //session.addOutput(depthDataOutput)
        session.addOutputWithNoConnections(depthDataOutput)
        depthDataOutput.setDelegate(self, callbackQueue: dataOutputQueue)
        //depthDataOutput.isFilteringEnabled = false
        if let inputPorts = frontCameraDeviceInput.ports(for: .depthData,
                                                         sourceDeviceType: frontCamera.deviceType,
                                                         sourceDevicePosition: frontCamera.position).first {
            let depthDataOutputConnection = AVCaptureConnection(inputPorts: [inputPorts], output: depthDataOutput)
            if session.canAddConnection(depthDataOutputConnection) {
                session.addConnection(depthDataOutputConnection)
            }
        }
        ...
        guard session.canAddOutput(metadataOutput) else {
            print("Could not add the metadata")
            return false
        }
        session.addOutputWithNoConnections(metadataOutput)

        if let inputPorts = frontCameraDeviceInput.ports(for: .metadataObject,
                                                         sourceDeviceType: frontCamera.deviceType,
                                                         sourceDevicePosition: frontCamera.position).first {
            let metadataOutputConnection = AVCaptureConnection(inputPorts: [inputPorts], output: metadataOutput)
            if session.canAddConnection(metadataOutputConnection) {
                session.addConnection(metadataOutputConnection)
            }
        }
...
//        guard session.canAddOutput(metadataOutput) else { fatalError() }
...
//        }
        ...
        // synchronize outputs
        dataOutputSynchronizer = AVCaptureDataOutputSynchronizer(dataOutputs: [frontCameraVideoDataOutput, depthDataOutput, metadataOutput])
...
//        frontPhotoOutput.isDepthDataDeliveryEnabled = frontPhotoOutput.isDepthDataDeliverySupported
//        frontPhotoOutput.enabledSemanticSegmentationMatteTypes = frontPhotoOutput.availableSemanticSegmentationMatteTypes
//        let frontPhotoConnection = AVCaptureConnection(inputPorts: [frontCameraVideoPort], output: frontPhotoOutput)
...
//        settings.enabledSemanticSegmentationMatteTypes = frontPhotoOutput.enabledSemanticSegmentationMatteTypes
//        settings.isDepthDataDeliveryEnabled = frontPhotoOutput.isDepthDataDeliveryEnabled
//        frontPhotoOutput.capturePhoto(with: settings, delegate: self)
//
//        let backSettings = AVCapturePhotoSettings()
//        backSettings.enabledSemanticSegmentationMatteTypes = backPhotoOutput.enabledSemanticSegmentationMatteTypes
//        backSettings.isDepthDataDeliveryEnabled = backPhotoOutput.isDepthDataDeliveryEnabled
...
//		recordButton.isEnabled = false
        ...
        defer {
            DispatchQueue.main.async {
                ...
                if let recorder = self.metalRecorder {
                    self.updateRecordButtonWithRecordingState(recorder.isRecording)
                }
            }
        }

        let isRecording = metalRecorder?.isRecording ?? false
        if !isRecording {
             metalRecorder?.startRecording()
        } else {
            metalRecorder?.endRecording(completion: { (url) in
	...
	private func processVideoSampleBuffer(_ sampleBuffer: CMSampleBuffer, fromOutput videoDataOutput: AVCaptureVideoDataOutput) {
        ...
        if videoDataOutput == backCameraVideoDataOutput {
            rendererBack(sampleBuffer)
            processMYVideoSampleBuffer(sampleBuffer)
        } else if videoDataOutput == frontCameraVideoDataOutput {
            rendererFront(sampleBuffer)
        }
        return
	...
	private func processsAudioSampleBuffer(_ sampleBuffer: CMSampleBuffer, fromOutput audioDataOutput: AVCaptureAudioDataOutput) {
        ...
        if let recorder = metalRecorder, audioDataOutput == backMicrophoneAudioDataOutput {
